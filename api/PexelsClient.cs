...
public class PexelsClient(HttpClient client, IConfiguration config)
{
    public async Task<IReadOnlyList<PexelsPhoto>> Search(string query, int perPage, CancellationToken ct)
    {
        ...
        {
            throw new InvalidOperationException("Pexels API key has not been configured.");
        }

        using var request = new HttpRequestMessage(HttpMethod.Get,
            $"https://api.pexels.com/v1/search?query={Uri.EscapeDataString(query)}&per_page={Math.Clamp(perPage, 1, 30)}&orientation=portrait");
