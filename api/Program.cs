    ...
    new StaticFileOptions
    {
        FileProvider =
            new Microsoft.Extensions.FileProviders.PhysicalFileProvider(
                Path.GetFullPath(builder.Configuration["Storage:LocalPath"] ?? "generated")),
...
{
    var u = await db.Users.Include(x => x.Interests).SingleOrDefaultAsync(x => x.Email == r.Email.ToLower());
    if (u is null || new PasswordHasher<AppUser>().VerifyHashedPassword(u, u.PasswordHash, r.Password) ==
...
app.MapGet("/api/profile", async (ClaimsPrincipal p, AppDbContext db) =>
{
    var u = await CurrentUser(p, db);
    return Results.Ok(ToDto(u));
}).RequireAuthorization();
app.MapPut("/api/profile", async (ProfileRequest r, ClaimsPrincipal p, AppDbContext db) =>
{
    var u = await CurrentUser(p, db);
    u.DisplayName = r.DisplayName.Trim();
    u.Interests = await db.Interests.Where(i => r.InterestIds.Contains(i.Id)).ToListAsync();
    await db.SaveChangesAsync();
    return Results.Ok(ToDto(u));
}).RequireAuthorization();
app.MapGet("/api/images/search",
        async (string q, PexelsClient client, CancellationToken ct) => Results.Ok(await client.Search(q, 18, ct)))
    .RequireAuthorization();
app.MapPost("/api/admin/quotes",
    async (QuoteRequest r, ClaimsPrincipal p, AppDbContext db, ImageComposer composer, CancellationToken ct) =>
    {
        var u = await CurrentUser(p, db);
        if (!u.IsAdmin)
        {
        ...
        {
            return Results.BadRequest(new { message = "A quote is required." });
        }

        var finalUrl = await composer.ComposeAndStore(r.ImageUrl, r.Quote.Trim(), ct);
        ...
        db.QuoteImages.Add(quote);
        await db.SaveChangesAsync(ct);
        return Results.Created($"/api/quotes/{quote.Id}", quote);
    }).RequireAuthorization();
app.MapGet("/api/quotes",
    async (AppDbContext db) => await db.QuoteImages.OrderByDescending(q => q.CreatedAt).Take(30).ToListAsync());
app.Run();

static async Task<AppUser> CurrentUser(ClaimsPrincipal p, AppDbContext db) => await db.Users.Include(x => x.Interests)
    .SingleAsync(u => u.Id == Guid.Parse(p.FindFirstValue(ClaimTypes.NameIdentifier)!));

static UserDto ToDto(AppUser u) => new(u.Id, u.Email, u.DisplayName, u.IsAdmin, u.Interests.Select(i => i.Id));

static AuthResponse CreateAuth(AppUser u, IConfiguration c)
{
    var claims = new[]
    {
        new Claim(ClaimTypes.NameIdentifier, u.Id.ToString()), new Claim(ClaimTypes.Email, u.Email),
        new Claim("admin", u.IsAdmin.ToString())
    };
    var key = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(c["Jwt:Key"]!));
    var token = new JwtSecurityToken(c["Jwt:Issuer"], c["Jwt:Audience"], claims, expires: DateTime.UtcNow.AddDays(7),
        signingCredentials: new SigningCredentials(key, SecurityAlgorithms.HmacSha256));
    return new(new JwtSecurityTokenHandler().WriteToken(token), ToDto(u));
}
