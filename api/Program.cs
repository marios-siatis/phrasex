...
app.UseStaticFiles(new StaticFileOptions { FileProvider = new Microsoft.Extensions.FileProviders.PhysicalFileProvider(Path.GetFullPath(builder.Configuration["Storage:LocalPath"] ?? "generated")), RequestPath = "/generated" });
...
app.MapPost("/api/auth/register", async (RegisterRequest r, AppDbContext db) => { if (await db.Users.AnyAsync(u => u.Email == r.Email.ToLower())) return Results.Conflict(new { message = "Email is already registered." }); var u = new AppUser { Email = r.Email.ToLower(), DisplayName = r.DisplayName }; u.PasswordHash = new PasswordHasher<AppUser>().HashPassword(u, r.Password); db.Users.Add(u); await db.SaveChangesAsync(); return Results.Ok(CreateAuth(u, builder.Configuration)); });
